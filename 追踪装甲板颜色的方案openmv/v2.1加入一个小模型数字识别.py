...
while True:
    ...
    for obj in tf.classify(net, img, min_scale=1.0, scale_mul=0.5, x_overlap=0.0, y_overlap=0.0):
        number = obj.output().index(max(obj.output()))
        rect = obj.rect()
        center = (rect[0] + rect[2] // 2, rect[1] + rect[3] // 2)
        radius = max(rect[2] // 2, rect[3] // 2)

        # 用圆圈圈出目标（下次再改）
        img.draw_circle(center[0], center[1], radius, color=(0, 255, 0))
        # 标注目标
        img.draw_string(center[0] - 10, center[1], f"Digit: {number}", color=(255, 255, 255), mono_space=False)
        print("Number:", number)
    '''
    for obj in tf.classify(net, img, min_scale=1.0, scale_mul=0.5, x_overlap=0.0, y_overlap=0.0):
        output = obj.output()
        number = output.index(max(output))
         # 在图像上框出数字
        img.draw_rectangle(obj.rect(), color=(0, 255, 0))  # 框出数字的矩形区域
        # 在图像上显示分类结果和概率
        img.draw_string(5, 5, f"Digit: {number}", color=(255, 255, 255), mono_space=False)
'''

    # 显示帧率
    img.draw_string(5, 15, "FPS: " + str(clock.fps()), color=(255, 255, 255), mono_space=False)
    print(clock.fps(), "fps")
